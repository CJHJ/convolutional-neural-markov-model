    ...
    def init_hidden(self, batch_size, hidden, shape):
        if self.Wci is None:
            self.Wci = Variable(torch.zeros(
                1, hidden, shape[0], shape[1])).to(self.device)
...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, kernel_size, pred_input_dim, device='cpu'):
        ...
        for i in range(self.num_layers):
            name = 'cell{}'.format(i)
            cell = ConvLSTMCell(
                self.input_channels[i], self.hidden_channels[i], self.kernel_size, device
            ).to(device)
            setattr(self, name, cell)
            self._all_layers.append(cell)

        # Prediction layer
        self.pred_conv = nn.Conv2d(self.pred_input_dim * self.hidden_channels[-1], self.input_channels[0],
                                   1, 1, 0, bias=True).to(device)
        self.tanh = nn.Tanh()
    ...
    def forward(self, input_tensor, pred_length, h_0=None, c_0=None):
        ...
        if pred_length == 0:
            internal_state = torch.zeros(
                bsize, seq_len + 1, self.hidden_channels[-1], height, width).to(self.device)
        else:
            internal_state = torch.Tensor(
                bsize, self.hidden_channels[-1] * self.pred_input_dim, height, width).to(self.device)

...
class Emitter(nn.Module):
    ...
    def __init__(self, input_channels, z_channels, emission_channels, kernel_size, bottleneck, n_layers):
        super().__init__()
        ...
        if bottleneck:
            ...
            for layer in range(n_layers - 1):
                ...
                self.deconv_layers.append(
                    nn.ConvTranspose2d(
                        emission_channels, next_emission_channels, kernel_size, stride, padding, bias=True).cuda()
                )
                emission_channels = next_emission_channels

            self.lin_hidden_to_input_loc = nn.ConvTranspose2d(
                emission_channels, input_channels, kernel_size, stride, padding, bias=True)
            ...
            for layer in range(n_layers - 1):
                self.deconv_layers.append(
                    nn.Conv2d(emission_channels, emission_channels,
                              kernel_size, 1, padding, bias=True).cuda()
                )
    ...
    def forward(self, z_t):
        ...
        scale = self.softplus(
            self.lin_hidden_to_input_scale(h1)).clamp(min=scale_clamper)
        return loc, scale
...
class GatedTransition(nn.Module):
    ...
    def forward(self, z_t_1):
        ...
        scale = self.softplus(self.lin_sig(
            self.relu(proposed_mean))).clamp(min=scale_clamper)
...
class Combiner(nn.Module):
    ...
    def forward(self, z_t_1, h_rnn):
        ...
        scale = self.softplus(
            self.lin_hidden_to_scale(h_combined)).clamp(min=scale_clamper)
        # return loc, scale which can be fed into Normal
        return loc, scale
...
class ObservationEncoder(nn.Module):
    ...
    def __init__(self, width, height, input_channels, encoder_channels, kernel_size, n_layers):
        super().__init__()
        ...
        for layer in range(n_layers):
            ...
            self.conv_layers.append(nn.Conv2d(
                encoder_channels, next_encoder_channels, kernel_size, stride, padding, bias=True).cuda())
...
class DMM(nn.Module):
    ...
    def __init__(self, input_channels=1, z_channels=16, emission_channels=32,
                 transition_channels=32, rnn_channels=32, encoder_channels=32, kernel_size=3, height=100, width=100, pred_input_dim=5, encoder_layer=2, num_layers=2, rnn_dropout_rate=0.0,
        ...
        self.emitter = Emitter(input_channels, z_channels,
                               emission_channels, kernel_size, self.bottleneck, encoder_layer)
        self.trans = GatedTransition(
            z_channels, transition_channels, kernel_size, width, height)
        self.combiner = Combiner(z_channels, rnn_channels[0], kernel_size)
        if self.bottleneck:
            self.obs_encoder = ObservationEncoder(
                width, height, input_channels, self.encoder_channels, kernel_size, encoder_layer)
        ...
        self.convlstm = ConvLSTM(
            self.rnn_input_channels, rnn_channels, kernel_size, pred_input_dim, self.device)
    ...
    def model(self, mini_batch, mini_batch_reversed, mini_batch_mask, annealing_factor=1.0):
        ...
        z_prev = self.z_0.expand(mini_batch.size(0), self.z_0.size(
            0), self.z_0.size(1), self.z_0.size(2))
    ...
    def guide(self, mini_batch, mini_batch_reversed, mini_batch_mask, annealing_factor=1.0):
        ...
        h_0 = self.h_0.expand(mini_batch.size(0), self.h_0.size(
            0), self.h_0.size(1), self.h_0.size(2))
        c_0 = self.c_0.expand(mini_batch.size(0), self.c_0.size(
            0), self.c_0.size(1), self.c_0.size(2))
        ...
        if self.bottleneck:
            ...
            enc_mini_batch_reversed = torch.zeros(
                batch_size, seq_len, self.rnn_input_channels, self.h_0.size(1), self.h_0.size(2)).to(self.device)
        ...
        z_prev = self.z_q_0.expand(mini_batch.size(0), self.z_q_0.size(
            0), self.z_q_0.size(1), self.z_q_0.size(2))
        ...
        with pyro.plate("z_minibatch", len(mini_batch)):
            ...
            for t in pyro.markov(range(1, T_max + 1)):
                ...
                assert z_dist.batch_shape == (
                    len(mini_batch), self.z_q_0.size(0), self.z_q_0.size(1), self.z_q_0.size(2))

                # sample z_t from the distribution z_dist
                with pyro.poutine.scale(scale=annealing_factor):
                    # when no normalizing flow used, ".to_event(1)" indicates latent dimensions are independent
                    z_t = pyro.sample("z_%d" % t,
                                      z_dist.to_event(dependent_event_dim))

                    assert z_t.shape == (len(mini_batch), self.z_q_0.size(
                        0), self.z_q_0.size(1), self.z_q_0.size(2))
    ...
    with torch.no_grad():
        ...
        for i in range(0, pred_length):
            ...
            z_t = pyro.sample("z_pred_%d" % i, dist.Normal(
                z_pred_loc, z_pred_scale).to_event(dependent_event_dim))
...
if __name__ == '__main__':
    ...
    dmm = DMM(input_channels=input_channels, z_channels=z_channels, emission_channels=emission_channels,
              transition_channels=transition_channels, rnn_channels=rnn_channels, kernel_size=kernel_size, height=height, width=width, pred_input_dim=5, num_layers=2, rnn_dropout_rate=0.0,

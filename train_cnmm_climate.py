...
from models.convdmm_unlayered import DMM, reverse_sequences, do_prediction, do_prediction_rep_inference
...
def main(args):
    '''
    Constants:
        time_length: Whole time length of the data
    '''
    # Constants
    time_length = 30
    ...
    lr_decay = 1.0
    weight_decay = 0
    parameter_codename = 'layer{}-lr{}-b{}-e{}-t{}-z{}-r{}-e{}'.format(
        encoder_layers, learning_rate, train_batch_size, emission_channels, transition_channels, z_channels, rnn_channels, encoder_channels)

    # Init tensorboard
    writer = SummaryWriter('./runs/' + args.runname +
                           parameter_codename + str(args.trialnumber))
    model_name = 'ConvDMM'
    ...
    train_valid_dataset = CMAPDataset(torch.Tensor(temp_train_valid_data))
    ...
    train_valid_dataloader = DataLoader(
        train_valid_dataset, batch_size=valid_batch_size, num_workers=8)
    ...
    dmm = DMM(input_channels=input_channels, z_channels=z_channels, emission_channels=emission_channels,
              transition_channels=transition_channels, rnn_channels=rnn_channels, encoder_channels=encoder_channels, kernel_size=kernel_size, height=height, width=width, pred_input_dim=pred_input_dim, encoder_layer=encoder_layers, num_layers=1, rnn_dropout_rate=0.0,
    ...
    save_model = Path('./checkpoints/ConvDMMClimate')
    ...
    if start_epoch != 0:
        load_opt = './checkpoints/ConvDMMClimate/e{}-i119-tn{}-opt.opt'.format(
            start_epoch - 1, args.trialnumber)
        load_model = './checkpoints/ConvDMMClimate/e{}-i119-tn{}.pt'.format(
            start_epoch - 1, args.trialnumber)
    ...
    if not validation_only:
        ...
        # Default 1000 testing 3000
        annealing_epochs = 1000.0
        # Default 0.2
            ...
            if epoch % 10 == 0 or epoch == epochs - 1:
                def validate_dataset(dataloader, name='test'):
                    for temp_pred_length in validation_pred_lengths:
                        ...
                        dmm.convlstm.eval()
                        with torch.no_grad():
                            for i, data in enumerate(tqdm(dataloader, desc=name, leave=True)):
                                data['observation'] = normalize(
                                    data['observation'].unsqueeze(2).to(device), data_min_val, data_max_val)
                                batch_size, length, _, w, h = data['observation'].shape
                            ...
                            writer.add_scalar(
                                'Loss/{}'.format(name), valid_loss_avg, epoch)
                            writer.add_scalar(
                                'Loss/{}_obs'.format(name), valid_loss_loc_avg, epoch)
                            logging.info("Validation loss: %1.5f",
                                         valid_loss_avg)
                            logging.info("Validation obs loss: %1.5f",
                                         valid_loss_loc_avg)
                            log_evaluation(evaluation_logpath, "Validation obs loss for {}s pred {} ConvDMM: {}\n".format(
                                val_pred_length, args.trialnumber, valid_loss_loc_avg))

                # Valid data
                validate_dataset(valid_dataloader, 'valid')

            # Save model
            if epoch == epochs - 1 and args.savemodel:
                torch.save(dmm.state_dict(), args.modelsavepath / 'ConvDMMClimate' /
                           'e{}-i{}-tn{}.pt'.format(epoch, idx, args.trialnumber))
                adam.save(args.modelsavepath / 'ConvDMMClimate' /
    ...
    if validation_only:
        ...
        with torch.no_grad():
            for i in tqdm(test_samples_indices, desc='Valid', leave=True):
                ...
                if i < 5:
                    ...
                    with open(save_dir_samples / '{}-dmm-pred.pkl'.format(i), 'wb') as fout:
        ...
        with open('ConvDMMResult.log', 'a+') as fout:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train ConvDMM model")
    ...
    parser.add_argument('--runname', '-rn', default="ConvDMM-Climate",
